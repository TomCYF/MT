...
class EncoderDecoder(Chain):
    ...
    def encode_list(self, in_word_list, train=True):
        ...
        for f_word, r_word in zip(var_en, var_rev_en):
            ...
            if first_entry == False:
                forward_states = F.concat((forward_states, self[self.lstm_rev_enc[-1]].h), axis=0)
    ...
    def select_word(self, prob, train=True, sample=False):
        ...
        if not sample:
            ...
            '''
            indx = xp.random.choice(range(len(prob)), p=prob.data[0])
            pred_word = Variable(xp.asarray([indx], dtype=np.int32), volatile=not train)
            pass
        return pred_word
        ...
        for next_word_var in var_dec[1:]:
            ...
            if self.attn == NO_ATTN:
                ...
                for state in enc_states:
                    t = F.matmul(ht,state)
                    e = F.exp(t)
                    t_list.append(e.data)
                    e_sum += e.data
                t_list = t_list/e_sum
                s = Variable(xp.asarray(t_list, dtype=np.float32).reshape((1,-1)), volatile='auto')
                w = F.tanh(self.attention(F.concat((self[self.lstm_dec[-1]].h,F.matmul(s,enc_states)),axis=1)))
                predicted_out = self.out(w)
                pass
            # compute loss
    ...
    def decoder_predict(self, start_word, enc_states, max_predict_len=MAX_PREDICT_LEN, sample=False):
        ...
        while pred_count < max_predict_len and (int(pred_word.data) != (EOS_ID)):
            ...
            if self.attn == NO_ATTN:
                ...
                s = Variable(xp.asarray(t_list, dtype=np.float32).reshape((1,-1)), volatile='auto')
                w = F.tanh(self.attention(F.concat((self[self.lstm_dec[-1]].h,F.matmul(s,enc_states)),axis=1)))
                alpha_arr = xp.vstack([alpha_arr,s.data])
                prob = F.softmax(self.out(w))
                pass
