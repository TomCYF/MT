...
class EncoderDecoder(Chain):
    ...
    def __init__(self, vsize_enc, vsize_dec,
        ...
        # dropout part
        # F.dropout(self['embed_enc'],ratio=0.8,train=True)
        ...
        The following lines build the structure of RNN.
        The first loop adds layers to encode source sentences from left-to-right, the second loop build RNN to 
            to encode from right-to-left.
        '''
        ...
        two inputs from encode part
        '''

        self.add_link("embed_dec", L.EmbedID(vsize_dec, 2*n_units))
        
        # dropout part
        # self[lstm_name](F.dropout(self['embed_dec'],ratio=0.8,train=True))
    ...
    '''
        ...
        c_state is the cell states of LSTM units of the last layer of encoder part(left-to-right and right-to-left)
    '''
    # F- chainer.function
    ...
    def feed_lstm(self, word, embed_layer, lstm_layer_list, train):
        ...
        embed_id = F.dropout(embed_layer(word),ration=0.8,train=True)
    ...
    def encode_list(self, in_word_list, train=True):
        ...
        for f_word, r_word in zip(var_en, var_rev_en):
            ...
            since we have two part of encoder
            '''
    ...
    def select_word(self, prob, train=True, sample=False):
        ...
        if not sample:
            ...
            #print(prob)
            #index = np.random.choice(range(len(prob)))
            #indx = self.out(self.lstm_dec[-1].h)
            #print(prob.data[0])
