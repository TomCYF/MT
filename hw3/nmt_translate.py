...
def predict_sentence(line_num, line_fr, line_en=None, display=True, plot_name=None, p_filt=0, r_filt=0):
    ...
    pred_ids, alpha_arr, store_list = model.encode_decode_predict(fr_ids)
    pred_words = [i2w["en"][w].decode() for w in pred_ids]
    for word_seq in store_list:
        store_word = [i2w["en"][w].decode() for w in word_seq]
        print(store_word)
    # store_word = [i2w["en"][w].decode() for w in [word for word in sotre_list]]
    #print(store_list)
